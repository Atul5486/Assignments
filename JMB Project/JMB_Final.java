...
class Main {
    ...
    public static void main(String[] args) {
        ...
        while (!validInput) {
            System.out.println(RESET+BOLD+MAROON+"Please your choice"+RESET+BG+BLACK);
            try {
                ch = inp.nextInt();
                inp.nextLine();
                if (ch >= 1 && ch <=5) {
                    ...
                    System.out.println(RED+"Invalid range!"+BG);
                }
            } catch (java.util.InputMismatchException e) {
                System.out.println(RED+"Invalid input! Please enter a number."+BG);
                inp.nextLine();
            }
        }
            switch (ch) {
...
void vegetableType(int thaliChoice){
    ...
    void orderBreakfast() {
        boolean isAlreadyOrdered = false;
        int choice = getNumber(inp,1,breakfast.length,"Enter item number to order (0 to skip):");
    ...
    void orderMocktail() {
            ...
            while (!validInput) {
                 System.out.println(RESET+BOLD+"Enter item number to order (0 to skip):"+RESET+BG+BLACK);
                try {
                    choice = inp.nextInt(); // Use your Scanner object
                    if (choice >= 1 && choice <= mocktails.length) {
                        validInput = true;
                    } else {
                        System.out.println(RED+"Invalid range! Please enter a number between 1 and "+mocktails.length+BG);
                    }
                } catch (java.util.InputMismatchException e) {
                    System.out.println(RED+"Invalid input! Please enter a whole number."+BG);
...
void chineseItems(){
    ...
    String items[] = {"Veg Manchurian","Chilli Paneer","Hakka Noodles","Fried Rice","Spring Rolls","Schezwan Noodles","Chilli Mushroom","Crispy Veg"};
     System.out.println(RESET+BOLD + MAROON + "\n=== CHINESE FOOD ITEMS  ===" + RESET+BG+BLACK);
        ...
        do{
        ch=getNumber(inp,1,8,"Enter item number to order (0 to skip):");
        switch(ch){
            case 1:
            case 2:
            case 3:
    ...
    void soupSpecial(){
        ...
        v1=getNumber(inp,1,soup.length,"Enter 1 soup ");
        if(v1<=soup.length)
        ...
        v2=getNumber(inp,1,soup.length,"Enter 2 soup ");
        if(v1==v2){
            v2=getNumber(inp,1,soup.length,"Select another soup");
        }
        if(v2<=soup.length){
...
void starterSpecial(String[] selectedItems) {
    ...
    while (i < 3) {
        int choice = getNumber(inp,1,starter.length,"Enter Starter #");
    ...
    void welcomeDrink(String selectedItems[]){
        ...
        int ch;
        do{
        ch=getNumber(inp,1,drink.length,"Enter Your Choice");
        switch(ch){
            case 1:
            case 2:
    ...
    void soupOrDrink(String selectedItems[]){
        ...
        sd=getNumber(inp,1,4,"\n1.Soup\n2.Welcome Drink");
        switch(sd){
                case 1:soupMenu();
                int ch;
                do{
                ch=getNumber(inp,1,4,"Enter a choice ");
        switch(ch){
            case 1:
            case 2:
            case 3:
    ...
    void allVegitable(String selectedItems[]){
        ...
        int v1,v2;
        do{
        v1=getNumber(inp,1,veg.length, "Enter 1 vegitable ");
        if(v1<=veg.length)
        ...
        do{
        v2=getNumber(inp,1,veg.length,"Enter 2 vegitable ");
        if(v1==v2){
            v2=getNumber(inp,1,veg.length,"Select another one vegitable");
        }
        if(v2<=veg.length){
    ...
    void dalList(String selectedItems[]){
       ...
       String[] dal = {"Dal Makhani", "Tadka Dal", "Dal Fry", "Chana Dal", "Moong Dal", "Panchmel Dal", "Masoor Dal", "Dal Tadka with Butter", "Dal Bukhara", "Dal Palak"};
        ...
        do{
        ch=getNumber(inp,1,dal.length,"Enter Your Choice ");
        switch(ch){
            case 1:
            case 2:
            case 3:
    ...
    void iceCreamList(String selectedItems[]){
        ...
        do{
        ch=getNumber(inp,1,iceCream.length,"Enter Your Choice ");
        switch(ch){
            case 1:
            case 2:
            case 3:
    ...
    void riceList(String selectedItems[]){ 
        ...
        do{
        ch=getNumber(inp,1,rice.length,"Enter Your Choice ");
        switch(ch){
            case 1:
            case 2:
            case 3:
    ...
    void pastaList(String selectedItems[]){
        ...
        do{
        ch=getNumber(inp,1,pasta.length,"Enter Your Choice ");
        switch(ch){
            case 1:
            case 2:
            case 3:
    ...
    void coffeeList(String selectedItems[],int index){
        ...
        do{
        ch=getNumber(inp,1,coffee.length,"Enter Your Choice ");
    ...
    void starterList(String selectedItems[],int index){
        ...
        do{
       ch=getNumber(inp,1,starter.length,"Enter Your Choice ");
        switch(ch){
            case 1:
            case 2:
            case 3:
    ...
    void curdOrder(String selectedItems[]){
        ...
        int ch;
        do{
        ch=getNumber(inp,1,7,"Enter Your Choice ");
        switch(ch){
            case 1:
            case 2:
            case 3:
    ...
    void paneerOrder(String selectedItems[]){
        ...
        int ch;
        do{
        ch=getNumber(inp,1,paneerDish.length,"Enter Your Choice ");
        switch(ch){
            case 1:
            case 2:
            case 3:
    ...
    void paneerStarterOrder(String selectedItems[],int index){
        ...
        int ch;
        do{
        ch=getNumber(inp,1,paneerStarter.length,"Enter Your Choice ");
        switch(ch){
            case 1:
            case 2:
            case 3:
    ...
    void sweetsList(String selectedItems[],int index){
        ...
        int ch;
        do{
        ch=getNumber(inp,1,sweet.length,"Enter Your Choice ");
        switch(ch){
            case 1:
            case 2:
            case 3:
    ...
    void details(){
        ...
        do{
            ...
            while (!validInput) {
                try {
                    guest =getNumber(inp,1,1000,"Enter total number of guest : ");
                    if (guest >= 1 && guest <= 1000) {
                        validInput = true;
                    } else {
                        System.out.println(RED+"Invalid range!"+BG);
                    }
        ...
        while (true) {
            ...
            try {
                ...
                System.out.println(RED+"Invalid date format! Please enter date as dd-MM-yyyy"+BG);
            }
        }
    }
    void displayBill() {
        do{
        currGuest=getNumber(inp,1,1000,"How many guest present here (Max 1000)" );
        if(currGuest<0 || currGuest >1000)
        System.out.println(RED+"Invalid range"+BG);
        }while(currGuest<=0 || currGuest>1000);
        int extra=0;
        int rem=0;
        if(guest>=100){
            extra=5;
...
void snacksListC(String type[], int index1, int index2) {
    ...
    do {
        ch1=getNumber(inp,1,snacks.length,"Enter Your 1st Snacks");
    ...
    do {
        ch2 =getNumber(inp,1,snacks.length, "Enter your choice 2 (different from choice 1)");
...
void snacksList(String type[],int index){
    String[] snacks = {"Samosa", "Spring Rolls", "Paneer Pakora", "Veg Cutlet", "Aloo Tikki", "Onion Pakora", "Bread Roll", "Cheese Balls", "Hara Bhara Kabab", "French Fries"};
        ...
        do{
        ch=getNumber(inp,1,snacks.length, "Enter your choice");
...
int getNumber(Scanner inp,int start,int end,String str) {
        boolean validInput = false;
        int choice = -1;

        while (!validInput) {
            System.out.println(RESET+BOLD+MAROON+str+RESET+BG+BLACK);
            try {
                choice = inp.nextInt();
                if (choice >= start && choice <= end) {
                    validInput = true;
                } else {
                    System.out.println(RED+"Invalid range! Please enter a number in range"+BG);
                }
            } catch (java.util.InputMismatchException e) {
                System.out.println(RED+"Invalid input! Please enter a number."+BG);
                inp.nextLine(); // clear invalid input
